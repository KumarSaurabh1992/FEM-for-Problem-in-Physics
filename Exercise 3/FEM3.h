...
double FEM<dim>::C(unsigned int i,unsigned int j,unsigned int k,unsigned int l){
  ...
  double E=2.0*100000000000 ,  //EDIT
    nu=0.3 ; //EDIT
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int globalNode = 0; globalNode <totalDOFs; globalNode++){
    if(fabs(dofLocation[globalNode][2] - 0) < 0.000001){
	...
	for(unsigned int i=0; i<dim; i++){ //Loop over nodal dofs
	  for (unsigned int B=0; B<nodes_per_elem; B++) {
		...
		for (unsigned int l = 0; l<dim; l++){
		  ...
		  Klocal[3*A+i][3*B+k] += fe_values.shape_grad(3*A+i,q)[j]*fe_values.shape_grad(3*B+k,q)[l]*C(i,j,k,l)*fe_values.JxW(q);
		}
	      }
	    }
	  }
