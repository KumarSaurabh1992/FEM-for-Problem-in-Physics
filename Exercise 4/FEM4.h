...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int globalNode = 0; globalNode <totalNodes; globalNode++){
    if(fabs(nodeLocation[globalNode][0] - 0) < 0.000001){
      boundary_values_of_D[globalNode] = 300;
      boundary_values_of_V[globalNode] = 0;
    }
    if(fabs(nodeLocation[globalNode][0] - 1.0) < 0.000001){
      boundary_values_of_D[globalNode] = 310;
      boundary_values_of_V[globalNode] = 0;
    }
  }
...
void FEM<dim>::assemble_system(){
  ...
  double		    rho = 3.8151*1000000;                            //EDIT - specify the specific heat per unit volume
	...
	for(unsigned int q=0; q<num_quad_pts; q++){
	  for(unsigned int i=0; i<dim; i++){
	    for(unsigned int j=0; j<dim; j++){
	      ...
	      //	      basis_gradient(B,quad_points[q1],quad_points[q2])[j]*invJacob[j][J]*kappa[I][J]*
	      //      quad_weight[q1]*quad_weight[q2]*detJ;
	      Klocal[A][B] += fe_values.shape_grad(A,q)[i]*fe_values.shape_grad(B,q)[j]*fe_values.JxW(q)*kappa[i][j];
	    }
	  }
	}
      }
...
void FEM<dim>::apply_initial_conditions(){
  ...
  for(unsigned int i=0; i<totalNodes; i++){
    if(nodeLocation[i][0] < 0.5){
      ...
      D_trans[i] = 300 + 20*(nodeLocation[i][0]-0.5); //EDIT
    }
  }
...
void FEM<dim>::solve_trans(){
  ...
  const unsigned int totalNodes = dof_handler.n_dofs(); //Total number of nodes
  Vector<double>     D_tilde(totalNodes),temp(totalNodes);
  ...
  for(unsigned int t_step=1; t_step<3001; t_step++){
    ...
    //EDIT
    D_tilde = D_trans;
    D_tilde.add((1-alpha)*delta_t,V_trans);
