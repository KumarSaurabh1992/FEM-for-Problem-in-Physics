...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  //EDIT
  unsigned  int i;
  for(i=0;i<(basisFunctionOrder+1);i++){
    if(i!=node){
      value = value*((xi-xi_at_node(i))/(xi_at_node(node)-xi_at_node(i)));
}
}
  //   std::cout << node  <<"\t" << xi << ":Val " << value << std::endl;
  return value;
}
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  value = value*basis_function(node,xi);
  }
  return value;
}
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	//EDIT - Define Flocal.
	Flocal[A] += (h_e/2)*basis_function(A,quad_points[q])*x*quad_weight[q];
	...
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
	  Klocal[A][B] += quad_weight[q]*basis_gradient(A,quad_points[q])*basis_gradient(B,quad_points[q])*2/h_e;
	}
      }
    }
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      if(prob == 1){
	u_exact = (-x*x*x/6) + (7*x/600);
	l2norm += quad_weight[q]*(u_exact - u_h)*(u_exact - u_h)*h_e/2;
      }
       if(prob == 2){
	 u_exact = (-x*x*x/6) + (21*x/200);
	 l2norm += quad_weight[q]*(u_exact - u_h)*(u_exact - u_h)*h_e/2;
       }					
    }
  }
