...
void FEM<dim>::define_boundary_conds(){
  ...
  for(unsigned int globalNode = 0; globalNode < totalNodes; globalNode++){
     if(fabs(nodeLocation[globalNode][0] - 0) < 0.000001){
       boundary_values[globalNode] = 300;
    }
    if(fabs(nodeLocation[globalNode][0] - 1) < 0.000001){
    boundary_values[globalNode] = 310;
    }
  }
}
	  ...
	  for(unsigned int A=0; A<dofs_per_elem; A++){
		...
		for(unsigned int j=0;j<dim;j++){
		  for(unsigned int I=0;I<dim;I++){
			...
			basis_gradient(B,quad_points[q1],quad_points[q2],quad_points[q3])[j]*invJacob[j][J]*kappa[I][J]*
			quad_weight[q1]*quad_weight[q2]*quad_weight[q3]*detJ;
		    }
		  }
		}
	      }
